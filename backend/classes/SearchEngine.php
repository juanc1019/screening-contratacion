...
class SearchEngine
{
    ...
    public function searchIndividual(
        ...
        $minSimilarity = $minSimilarity ?? $this->config['search']['similarity']['default_threshold'];
    ...
    private function searchByName(
        ...
        $identification = $identification ? $this->cleanSearchTerm($identification) : null;
    ...
    private function searchByIdentification(string $identification, float $minSimilarity = 80.0, int $limit = 50): array
    {
        ...
        $exactResults = $stmt->fetchAll();
        
        // Si no hay resultados exactos, buscar por similitud
        if (empty($exactResults) && $minSimilarity < 100) {
    ...
    {
        $nameScore = (float)($result['name_similarity'] ?? 0);
        $idScore = (float)($result['id_similarity'] ?? 0);
        
        if ($nameScore >= 100 && $idScore >= 100) {
            return 'exact';
        } elseif ($nameScore >= 95 || $idScore >= 95) {
            return 'high_similarity';
        } elseif ($nameScore >= 80 || $idScore >= 80) {
    ...
    {
        $combinedScore = (float)($result['combined_score'] ?? 0);
        
        if ($combinedScore >= 95) {
            return 'very_high';
        } elseif ($combinedScore >= 85) {
            return 'high';
    ...
    public function advancedSearch(array $criteria): array
    {
        ...
        try {
            ...
            $sql = "SELECT *, 
                    CASE WHEN ? != '' THEN calculate_similarity(full_name, ?) ELSE 0 END as name_similarity,
                    CASE WHEN ? != '' THEN calculate_similarity(identification, ?) ELSE 0 END as id_similarity
                    FROM local_database_records 
                    WHERE is_active = true AND (" . implode(' OR ', $whereConditions) . ")
                    ORDER BY 
                        GREATEST(
                            CASE WHEN ? != '' THEN calculate_similarity(full_name, ?) ELSE 0 END,
                            CASE WHEN ? != '' THEN calculate_similarity(identification, ?) ELSE 0 END
                        ) DESC
                    LIMIT ?";
            ...
            $finalParams[] = $criteria['identification'] ?? '';
            $finalParams[] = $criteria['limit'] ?? 50;
            
            $stmt = $this->db->query($sql, $finalParams);
            $results = $stmt->fetchAll();
            
            $processed = $this->processSearchResults($results, $criteria['name'] ?? '', $criteria['identification'] ?? '');
    ...
    public function getSearchStats(): array
    {
        return array_merge($this->searchStats, [
            'cache_hit_ratio' => $this->searchStats['searches_performed'] > 0 
                ? round(($this->searchStats['cache_hits'] / $this->searchStats['searches_performed']) * 100, 2)
                : 0,
            'average_execution_time_ms' => $this->searchStats['searches_performed'] > 0
                ? round($this->searchStats['execution_time_ms'] / $this->searchStats['searches_performed'], 2)
                : 0
        ]);
    }
